    ...
    <div className="relative flex flex-col min-h-[80vh] font-sans">
      ...
      <div className="flex-1">
        ...
        {currentView === 'prescriptions' && (
          <div className="max-w-4xl mx-auto animate-fade-in mt-8 space-y-8">
            <section className="bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-xl border border-gray-100">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                  <h3 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
                    <span className="text-4xl">💊</span> Medical Records
                  </h3>
                  <p className="text-gray-500 mt-2">Manage your past prescriptions, lab reports, and medical history.</p>
                </div>
              </div>

              {/* Upload Form */}
              <div className="bg-indigo-50/50 rounded-3xl p-6 md:p-8 mb-10 border border-indigo-100">
                <h4 className="text-lg font-bold text-indigo-900 mb-4 flex items-center gap-2">
                  <span>📤</span> Upload New Record
                </h4>
                <form onSubmit={handleAddPrescription} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <input
                      type="text"
                      placeholder="Record Title (e.g. Blood Test, X-Ray)"
                      value={prescriptionForm.title}
                      onChange={e => setPrescriptionForm({...prescriptionForm, title: e.target.value})}
                      className="w-full p-4 rounded-2xl border border-white bg-white shadow-sm outline-none focus:ring-2 focus:ring-indigo-500"
                      required
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    <input
                      ...
                      className="w-full p-4 rounded-2xl border border-white bg-white shadow-sm outline-none focus:ring-2 focus:ring-indigo-500 text-gray-500"
                      required
                    />
                  </div>
                  <div>
                    <input
                      ...
                      className="w-full p-3 rounded-2xl border border-white bg-white shadow-sm outline-none focus:ring-2 focus:ring-indigo-500 text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-indigo-100 file:text-indigo-700 hover:file:bg-indigo-200 transition-colors cursor-pointer"
                      required
                    />
                  </div>
                  <div className="md:col-span-2 mt-2">
                    <button type="submit" className="w-full py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold transition-all shadow-md active:scale-95">
                      Save Record
                    </button>
                  </div>
                </form>
              ...
              {/* Records List */}
              <div className="space-y-4">
                <h4 className="text-lg font-bold text-gray-800 mb-4 px-2">Your Documents</h4>
                {prescriptions.length === 0 ? (
                  <div className="p-8 bg-gray-50 rounded-2xl border border-gray-100 text-center text-gray-400 italic">
                  ...
                  prescriptions.map(record => (
                    <div key={record._id || record.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-5 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h5 className="font-bold text-gray-800 text-lg">{record.title}</h5>
                          <div className="flex flex-wrap items-center gap-2 text-sm text-gray-500 mt-1">
                            <span className="font-medium text-indigo-600">{record.doctorName}</span>
                            <span>•</span>
